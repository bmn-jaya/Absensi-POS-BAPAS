...
function buildHtml(page) {
  ...
  if (page === "admin") {
     body = getAdminHTML();
     js = getAdminJS() + getAdminJS2() + getAdminJS3() + getAdminJS4();
...
function getAdminJS4() {
  ...
  js += "function downloadIDCard(id){var e=employees.find(function(x){return x.id===id;});if(!e){alert('Data not found');return;}";
  js += "var div=document.createElement('div');div.id='qr-temp';div.style.display='none';document.body.appendChild(div);";
  js += "new QRCode(div,{text:e.nik,width:128,height:128});";
  js += "setTimeout(function(){var qr=div.querySelector('img').src;document.body.removeChild(div);genPDF(e,qr);},500);}";
  js += "function genPDF(e,qr){var {jsPDF}=window.jspdf;var doc=new jsPDF({orientation:'landscape',unit:'mm',format:[85,55]});";
  js += "doc.setFillColor(59,130,246);doc.rect(0,0,5,55,'F');";
  js += "doc.setFontSize(14);doc.setFont('helvetica','bold');doc.text(e.nama,10,15);";
  js += "doc.setFontSize(10);doc.setFont('helvetica','normal');doc.setTextColor(100);doc.text(e.divisi,10,21);";
  js += "doc.setDrawColor(200);doc.line(10,25,80,25);";
  js += "doc.addImage(qr,'PNG',60,28,20,20);";
  js += "doc.setFontSize(8);doc.setTextColor(50);doc.text('NIK: '+e.nik,10,35);doc.text('ID: '+e.id,10,40);";
  js += "doc.setFontSize(6);doc.setTextColor(150);doc.text('LevelUp HR Portal',10,50);";
  js += "doc.save('IDCard_'+e.nama+'.pdf');}";
  return js;
}
...
function getEmployeeJS() {
  ...
  js += "function showInput(){stopClock();document.getElementById('app-container').innerHTML='<div class=\"input-screen\"><div class=\"logo-box\" style=\"margin-bottom:20px\"><i data-lucide=\"hexagon\" style=\"width:24px\"></i></div><h1 class=\"welcome-text\">Attendance Pro</h1><p class=\"welcome-sub\">Enter your Employee ID to access the attendance portal.</p><div class=\"form-group\"><input type=\"text\" class=\"form-input\" id=\"nik-in\" placeholder=\"Employee ID (NIK)\"></div><button class=\"btn-primary\" onclick=\"checkEmp()\">Get Started</button><div style=\"margin-top:30px;display:flex;flex-direction:column;gap:10px;text-align:center\"><a href=\"?page=register\" style=\"font-size:12px;color:var(--primary);text-decoration:none\"><i data-lucide=\"user-plus\" style=\"width:12px;vertical-align:middle\"></i> Register New Employee</a><a href=\"?page=admin\" style=\"font-size:12px;color:var(--text-muted);text-decoration:none\"><i data-lucide=\"lock\" style=\"width:12px;vertical-align:middle\"></i> Login as Admin</a></div></div>';lucide.createIcons();}";
...
function handleSelfRegister(d) {
  ...
  if(found) {
     // UPDATE EXISTING
     s.getRange(rowIdx, 3).setValue(d.nama); // Nama
     // s.getRange(rowIdx, 4).setValue(d.divisi); // Keep existing division or update? User didn't ask for division input in UI but plan had it. Let's assume update if provided.
     s.getRange(rowIdx, 5).setValue(d.foto); // Foto URL
     s.getRange(rowIdx, 7).setValue(JSON.stringify(d.desc)); // Face Descriptor
  } else {
     // INSERT NEW
     var id = "EMP" + Date.now();
     // ID, NIK, Nama, Divisi, Foto, Status, Desc
...
function getAttendanceLog(){
  try {
      ...
      var d_disp = range.getDisplayValues(); // Fetch display values specifically for Time columns
      var arr=[];
      ...
      for(var j=0; j<d.length; j++){
         if(d[j][0] && String(d[j][0]).length > 0) validIndices.push(j);
      }
      
      var count = validIndices.length;
      var processStart = 1; 
      if(count > 501) processStart = count - 500;
      if(count <= 1) return {success:true, attendance:[]}; 

      for(var k=processStart; k<count; k++){
        var i = validIndices[k]; // Use actual row index
        ...
        // USE DISPLAY VALUES FOR TIME (to avoid 1899 Date Object Timezone shifts)
        var jMStr = d_disp[i][5];
        var jPStr = d_disp[i][6];
...
function getRegisterJS() {
    var js = "";
    js += "var uploadedImg=null; document.addEventListener('DOMContentLoaded', function(){lucide.createIcons(); loadFaceApi();});";
    js += "function loadFaceApi(){Promise.all([faceapi.nets.tinyFaceDetector.loadFromUri('https://cdn.jsdelivr.net/npm/@vladmandic/face-api/model'),faceapi.nets.faceLandmark68Net.loadFromUri('https://cdn.jsdelivr.net/npm/@vladmandic/face-api/model'),faceapi.nets.faceRecognitionNet.loadFromUri('https://cdn.jsdelivr.net/npm/@vladmandic/face-api/model')]).then(function(){console.log('Face API Ready');});}";
    js += "function previewFile(input){if(input.files && input.files[0]){var reader=new FileReader();reader.onload=function(e){var img=document.getElementById('preview-img');img.src=e.target.result;img.style.display='block';document.getElementById('placeholder-icon').style.display='none';uploadedImg=new Image();uploadedImg.src=e.target.result;};reader.readAsDataURL(input.files[0]);}}";
    js += "function submitReg(){var nama=document.getElementById('reg-nama').value;var nik=document.getElementById('reg-nik').value;if(!nama||!nik){alert('Mohon lengkapi Nama dan NIK');return;}if(!uploadedImg){alert('Mohon upload foto wajah');return;}";
    js += "var load=document.getElementById('loading-overlay');var txt=document.getElementById('load-text');load.style.display='flex';txt.textContent='Mendeteksi Wajah...';";
    js += "var cv=document.createElement('canvas');cv.width=uploadedImg.width;cv.height=uploadedImg.height;cv.getContext('2d').drawImage(uploadedImg,0,0);";
    js += "faceapi.detectSingleFace(cv,new faceapi.TinyFaceDetectorOptions()).withFaceLandmarks().withFaceDescriptor().then(function(d){if(!d){load.style.display='none';alert('Wajah tidak terdeteksi pada foto. Gunakan foto yang jelas.');return;}";
    js += "txt.textContent='Menyimpan Data...';var desc=Array.from(d.descriptor);";
    js += "var sm=document.createElement('canvas');sm.width=200;sm.height=200;sm.getContext('2d').drawImage(uploadedImg,0,0,200,200);var url=sm.toDataURL('image/jpeg',0.6);";
    js += "google.script.run.withSuccessHandler(function(r){load.style.display='none';if(r.success){alert('Berhasil Daftar!');window.location.reload();}else{alert('Gagal: '+r.message);}}).handleSelfRegister({nama:nama,nik:nik,foto:url,desc:desc});}).catch(function(e){load.style.display='none';alert('Error: '+e);});}";
    return js;
}
